...
/*
func lead_lag_offset(end int, config nsutil.Config_t) (pos int) {
	offset := float32(config.Fwin_len)/2.0 - float32(config.Lead_length)
	return end + int(float32(config.Symbol_size) * offset)
}
...
*/
	...
	plotter.CheckedCmd("set terminal png small size 1500,200")
